...
dojo.ready(function() {
	...
	function render() {
		...
		for ( var slot in slotMap) {
			...
			context.rotate(-Math.atan2(edge.N[1], edge.N[0]) + Math.PI / 2);
			context.clearRect(10, edge.N[1] * 60, 300, 50);
			context.fillText(slotMap[slot].name + '(' + (slotScore[slot] || 0) + ')', 10, edge.N[1] * 60);
			context.restore();
		}
